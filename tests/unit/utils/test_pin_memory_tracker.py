...
def _reset_tracker():
    # _tracker is a module-level singleton shared across every test here, so
    # without a guaranteed reset on both sides, a test running out of file
    # order (or a prior test failing before its own cleanup) leaks byte counts
